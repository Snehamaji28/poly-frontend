...
export default function CertificateValidatePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://rezime.in/validate/${certificateNumber}`);
      if (!response.ok) {
            ...
            {error ? (
              <div className="flex flex-col items-center text-center p-6">
                ...
                <p className="text-slate-400 max-w-md">
                  The certificate number you provided could not be verified. Please check the number and try again.
                </p>
              </div>
            ) : certificateData && (
              <div className="flex flex-col items-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-6">Valid Certificate</h3>
                <div className="grid md:grid-cols-2 gap-4 w-full max-w-2xl">
                  ...
                  <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
                    <p className="text-slate-400 text-sm mb-1">Certificate Type</p>
                    <p className="text-white font-medium">{certificateData.type}</p>
                  </div>
                  <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
                    <p className="text-slate-400 text-sm mb-1">Issue Date</p>
                    <p className="text-white font-medium">{certificateData.issueDate}</p>
                  </div>
                  <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
                    <p className="text-slate-400 text-sm mb-1">Certificate Number</p>
                    <p className="text-white font-medium">{certificateData.certificateNumber}</p>
                  </div>
                </div>
              </div>
            )}
